...
class domain_info:
    ...
    def domain_whois(self):
        ...
        if str(self.top_whois_server) == "['whois.verisign-grs.com', 'whois.crsnic.net']":
            ...
            self.gs_manage()
        elif str(self.top_whois_server) == "whois.tonic.to":
            self.to_manage()
        else:
            return
    ...
    def us_manage(self):
        ...
        pattern = re.compile(
            r'(Domain Name:.*|Registrant Phone Number:.*|Registrant Name:.*|Registrant Email:.*|Registrant E-mail:.*)')
        match = pattern.findall(data_result)
        match_length = len(match)
    ...
    def tc_manage(self):
        ...
        pattern = re.compile(
            r'(Domain Name:.*|Registrant Name:.*|Registrant Email:.*|Registrant Phone:.*)')
        match = pattern.findall(data_result)
        print match
        count = len(match)
    ...
    def pro_manage(self):
        ...
        if data_result:
            pattern = re.compile(
                r'(Domain Name:.*|Registrant Name:.*|Registrant Phone:.*|Registrant Email:.*)')
            match = pattern.findall(data_result)
            print 'match::', match
            count = len(match)
    ...
    def biz_manage(self):
        ...
        if data_result:
            pattern = re.compile(
                r'(Registrant Name:.*|Registrant Email:.*|Registrant Phone Number:.*)')
            match = pattern.findall(data_result)
            print match
            count = len(match)
    ...
    def mobi_manage(self):
        ...
        if data_result:
            pattern = re.compile(
                r'(Registrant Name:.*|Registrant Email:.*|Registrant Phone:.*)')
            match = pattern.findall(data_result)
            print match
            count = len(match)
...
def get_domain():
    ...
    sql = 'SELECT domain FROM whois_info_copy_copy where (reg_name=" " and reg_email=" " and reg_phone=" ") '
    cursor.execute(sql)
...
def check_domain(url=''):
    ...
    sql = 'UPDATE whois_info_copy_copy set top_whois_server="%s",sec_whois_server="%s",reg_name="%s",reg_email = "%s",reg_phone="%s" WHERE domain = "%s"' \
        % (str(query_domain.top_whois_server), query_domain.sec_whois_server, query_domain.reg_name, query_domain.reg_email, query_domain.reg_phone, query_domain.domain)
...
def main():
    ...
    while count * THREADNUM < total_domain_count:
        ...
        jobs = [gevent.spawn(check_domain, domain.strip())
